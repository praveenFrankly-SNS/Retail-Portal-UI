// ============================================================
// RelatedProducts — Legacy fallback display component
// Standardized keys to match Product Types.
// ============================================================
          ...
          return (
            ...
            >
              ...
              <div className="relative aspect-[4/3] bg-slate-50 border-b border-slate-100 flex flex-col items-center justify-center p-4 text-center">
                {rp.image_url ? (
                  <img src={rp.image_url} alt={rp.product_name || ''} className="w-full h-full object-cover" />
                ) : (
                  <>
                    ...
                    <div className="flex items-center gap-0.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          key={s}
                          size={12}
                          weight={rp.avg_rating! >= s ? 'fill' : 'regular'}
                          className={rp.avg_rating! >= s ? 'text-amber-400' : 'text-gray-300'}
                        />
                      ))}
                    </div>
                    <span className="text-xs text-gray-500 font-medium">
                      {rp.avg_rating!.toFixed(1)} ({rp.review_count})
                    </span>
                  </div>
                )}
