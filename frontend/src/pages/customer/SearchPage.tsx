...
// ── No Results — wireframe-accurate with AI search rescue row ─────────────────

function NoResultsState({ query }: { query: string }) {
  const { activeCustomer, sessionContext } = useUserStore();
  const [rescueRecs, setRescueRecs] = useState<RecommendedProduct[]>([]);
  const [selectedProduct, setSelectedProduct] = useState<RecommendedProduct | null>(null);
  const [recsLoading, setRecsLoading] = useState(true);

  useEffect(() => {
    setRecsLoading(true);
    // Inject current failed search query directly into searches context
    const rescueContext = {
      ...sessionContext,
      recent_searches: [query, ...sessionContext.recent_searches]
    };
    
    getRecommendations(activeCustomer.customer_id, 'EMPTY_SEARCH', null, 8, rescueContext)
      .then((res) => {
        setRescueRecs(res.recommendations);
      })
      .catch((err) => console.warn(err))
      .finally(() => setRecsLoading(false));
  }, [query, activeCustomer.customer_id]);
  ...
  return (
    <div className="space-y-10 w-full">
      ...
      {/* AI search rescue suggestions row */}
      {rescueRecs.length > 0 && (
        <div className="border-t border-slate-200/60 pt-8 mt-4">
          <RecommendationRow
            id="search-rescue"
            title="AI Search Rescue Suggestions"
            subtitle={`We couldn't find exact matches for "${query}", but based on ${activeCustomer.customer_name}'s profile, you might like these:`}
            items={rescueRecs}
            isAI
